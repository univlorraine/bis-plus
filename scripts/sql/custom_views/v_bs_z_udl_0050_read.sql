...
CREATE VIEW splus.v_bs_z_udl_0050_read AS
    ...
    t1.rev_refnr,
    t1.doctype
   FROM {target_schema}.fmbh t1
     LEFT JOIN {target_schema}.fmbl t2 ON t1.fm_area = t2.fm_area AND t1.docyear = t2.docyear AND t1.docnr = t2.docnr
  WHERE t1.process_ui = 'TRAN'::bpchar AND (t1.doctype = 'CVRA'::bpchar OR t1.doctype = 'REAJ'::bpchar OR t1.doctype = 'VIEX'::bpchar OR t1.doctype = 'VIIN'::bpchar) AND (t1.docstate = '1'::bpchar OR t1.docstate = '3'::bpchar)
UNION
 SELECT t1.docyear,
    t1.fm_area,
    t2.docnr,
    ...
    t1.doctype
   FROM {target_schema}.fmbh t1
     JOIN ( SELECT t3.rev_refnr
           FROM {target_schema}.fmbh t3
          WHERE t3.process_ui = 'TRAN'::bpchar AND (t3.doctype = 'CVRA'::bpchar OR t3.doctype = 'REAJ'::bpchar OR t3.doctype = 'VIEX'::bpchar OR t3.doctype = 'VIIN'::bpchar) AND (t3.docstate = '1'::bpchar OR t3.docstate = '3'::bpchar) AND t3.rev_refnr <> ' '::bpchar) t4 ON t1.docnr = t4.rev_refnr
     LEFT JOIN {target_schema}.fmbl t2 ON t1.fm_area = t2.fm_area AND t1.docyear = t2.docyear AND t1.docnr = t2.docnr
  WHERE t1.process_ui = 'TRAN'::bpchar AND (t1.doctype = 'CVRA'::bpchar OR t1.doctype = 'REAJ'::bpchar OR t1.doctype = 'VIEX'::bpchar OR t1.doctype = 'VIIN'::bpchar) AND (t1.docstate = '1'::bpchar OR t1.docstate = '3'::bpchar)
  ORDER BY 2, 3;
