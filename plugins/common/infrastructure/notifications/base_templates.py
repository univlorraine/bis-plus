...
class BaseTemplates:
    ...
    @staticmethod
    def build_info_grid(items: 'list[tuple[str, str]]') -> str:
        """Construit les paires <div class="info-label">/<div class="info-value"> d'un info-grid.

        Args:
            items: liste de (label, value) — `value` est inséré tel quel (le
                caller décide s'il faut échapper le texte ou inclure du HTML
                pré-construit, ex: un badge `<span class="badge">...`).
        """
        return "\n".join(
