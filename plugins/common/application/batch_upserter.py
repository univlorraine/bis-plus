...
Execution des UPSERTs SQL par batch.

Ce module gere l'insertion/mise à jour des donnees en base PostgreSQL par
batch, avec gestion des transactions et detection des conflits. Utilisé par
AMUE et ECC directement, via les points d'extension
_before_execute()/_interpret_results() pour les besoins spécifiques.
"""
import logging
import time
...
from common.domain.exceptions import BatchError, DatabaseError, DataError
...
class BatchUpserter:
    ...
    Example:
        >>> inserter = BatchUpserter(postgres_hook)
        >>> inserter.execute_batch(cursor, conn, sql, batch, table_name, columns, pks)
        >>> # Avec blue/green
        >>> inserter = BatchUpserter(postgres_hook, target_schema='splus_blue')
    """
    ...
    def execute_batch(
        ...
        Returns:
            Métriques du batch :
            {
                ...
                'rows_unchanged': int,  # lignes sans changement réel (0 sauf sous-classe)
                'changed_rows': list,   # lignes changées, prêt pour publication (vide sauf sous-classe)
                'batch_size': int,
        ...
        try:
            pre_state = self._before_execute(cursor, table_name, columns, primary_keys, batch)

            if primary_keys:
                # UPSERT avec RETURNING (xmax = 0) AS is_insert par défaut
                # xmax=0 → INSERT (nouvelle ligne), xmax≠0 → UPDATE (mise à jour)
                # — voir _interpret_results() pour le point d'extension.
                results = execute_values(cursor, insert_sql, batch,
                                         page_size=len(batch), fetch=True)
                rows_inserted, rows_updated, rows_unchanged, changed_rows = (
                    self._interpret_results(results, batch, columns, primary_keys, pre_state)
                )
            else:
                # INSERT simple sans RETURNING : toutes les lignes sont nouvelles
                execute_values(cursor, insert_sql, batch, page_size=len(batch))
                rows_inserted, rows_updated, rows_unchanged, changed_rows = len(batch), 0, 0, []
    ...
    def _before_execute(
        ...
        """
        Point d'extension appelé juste avant l'exécution du batch (avant
        ``execute_values``), après la détection de doublons.

        Base : no-op (retourne None). Une sous-classe peut l'utiliser pour
        précalculer un état nécessaire à ``_interpret_results`` (ex: lire les
        clés primaires déjà présentes en base pour distinguer insert/update
        sans dépendre de ``xmax``).
        """
        return None
    ...
    def _interpret_results(
        ...
        """
        Point d'extension : interprète les lignes RETURNED par
        ``execute_values`` en métriques (inserted/updated/unchanged) et
        lignes changées.

        Base : convention ``RETURNING (xmax = 0) AS is_insert`` — chaque ligne
        de ``results`` est un tuple à 1 élément (bool). Retourne toujours
        ``rows_unchanged=0`` et ``changed_rows=[]`` (pas de suivi de
        changement réel dans le comportement par défaut).
