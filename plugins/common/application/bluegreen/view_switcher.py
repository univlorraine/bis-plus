...
from common.infrastructure.database.connection_manager import PostgresConnectionManager
from common.infrastructure.database.schema_introspection import list_tables, list_views
from common.infrastructure.database.sql_file_loader import read_sql_file
...
class ViewSwitcher:
    ...
    def __init__(self, postgres_hook: SqlExecutor = None, custom_views_dir: Path = CUSTOM_VIEWS_DIR):
        """
        ...
        with PostgresConnectionManager(self.postgres_hook) as conn_mgr:
            ...
            try:
                if self.custom_views_dir.exists():
                    result = self._execute_custom_views(cursor, conn_mgr, target_schema, track_failures=True)
                    self._ok_custom_views_files = result["files_processed"]
                    self._failed_custom_views_detail = result["files_failed"]
                    self._failed_custom_views = result["failed_view_names"]
            except Exception as e:
                logger.warning(f"[VIEW_SWITCH] Erreur vues custom (non bloquant): {e}")
            finally:
                cursor.close()
            return True
    ...
    def refresh_custom_views(self, target_schema: str) -> Dict:
        ...
        with PostgresConnectionManager(self.postgres_hook) as conn_mgr:
            ...
            cursor = conn.cursor()
            try:
                result = self._execute_custom_views(cursor, conn_mgr, target_schema)
            finally:
                cursor.close()

        logger.info(f"[REFRESH_VIEWS] {result['ok']} OK, {result['ko']} en échec → {target_schema}")
        return {
            "ok": result["ok"],
            "ko": result["ko"],
    ...
    def _execute_custom_views(
        self, cursor, conn_mgr: PostgresConnectionManager, target_schema: str, track_failures: bool = False
    ) -> Dict:
        """
        Exécute un par un les fichiers .sql de custom_views_dir, en best-effort.

        Chaque fichier est exécuté et commité (ou rollback en cas d'erreur)
        indépendamment des autres, après vérification d'intégrité (manifest SHA-256)
        et substitution du placeholder `{target_schema}`.

        Args:
            cursor: Curseur PostgreSQL ouvert sur une connexion déjà obtenue via conn_mgr
            conn_mgr: Gestionnaire de connexion (commit/rollback par fichier)
            target_schema: Schéma cible à injecter (ex: 'splus_blue' ou 'splus_green')
            track_failures: Si True, extrait et retourne le nom des vues en échec
                (utilisé par verify_views_point_to pour exclure ces vues du contrôle)

        Returns:
            {
                "ok": int, "ko": int,
                "files_processed": List[str] (fichiers exécutés avec succès),
                "files_failed": List[Dict] ({"filename": ..., "error": ...}),
                "failed_view_names": Set[str] (noms de vues en échec, si track_failures),
            }
        """
        ok = ko = 0
        files_processed: List[str] = []
        files_failed: List[Dict] = []
        failed_view_names: Set[str] = set()

        for sql_file in sorted(self.custom_views_dir.glob("*.sql")):
            content = ""
            try:
                content = read_sql_file(sql_file).replace("{target_schema}", target_schema)
                ...
                if track_failures:
                    ...
                    if view_name:
                        failed_view_names.add(view_name)
        ...
        return {
            ...
            "failed_view_names": failed_view_names,
        }
    ...
    def _load_custom_view_sqls(self, target_schema: str) -> List[str]:
        ...
        if not self.custom_views_dir.exists():
            return sqls
        for sql_file in sorted(self.custom_views_dir.glob("*.sql")):
            content = read_sql_file(sql_file, verify_integrity=False)
