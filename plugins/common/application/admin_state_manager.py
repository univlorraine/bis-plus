...
from common.infrastructure.observability.tracing import to_iso_str

logger = logging.getLogger(__name__)

# Nom de table fixe (constante de compilation, jamais dynamique) : un simple
# littéral SQL suffit, pas besoin de psycopg2.sql.Identifier.
...
class AdminStateManager:
    ...
    def __init__(self, postgres_hook: Optional[SqlExecutor] = None):
        self._hook: SqlExecutor = postgres_hook or create_postgres_hook(schema='public')
    ...
    def get_last_finish_timestamp(self) -> Optional[str]:
        ...
        try:
            row = self._fetch_one(
                "SELECT last_finish_timestamp FROM {} WHERE id = %s".format(_TABLE),
                (_ROW_ID,)
            )
    ...
    def set_last_finish_timestamp(self, ts: str) -> None:
        ...
        try:
            self._run(
                "UPDATE {} SET last_finish_timestamp = %s, updated_at = NOW() WHERE id = %s".format(_TABLE),
                (ts, _ROW_ID)
            )
    ...
    def get_last_successful_run(self) -> Optional[str]:
        ...
        try:
            row = self._fetch_one(
                "SELECT last_successful_run FROM {} WHERE id = %s".format(_TABLE),
                (_ROW_ID,)
            )
            if row and row[0] is not None:
                return to_iso_str(row[0])
    ...
    def set_last_successful_run(self, ts: str) -> None:
        ...
        try:
            self._run(
                "UPDATE {} SET last_successful_run = %s, updated_at = NOW() WHERE id = %s".format(_TABLE),
                (ts, _ROW_ID)
            )
    ...
    def get_last_report_start(self) -> Optional[str]:
        ...
        try:
            row = self._fetch_one(
                "SELECT last_report_start FROM {} WHERE id = %s".format(_TABLE),
                (_ROW_ID,)
            )
            if row and row[0] is not None:
                return to_iso_str(row[0])
    ...
    def set_last_report_start(self, ts: str) -> None:
        ...
        try:
            self._run(
                "UPDATE {} SET last_report_start = %s, updated_at = NOW() WHERE id = %s".format(_TABLE),
                (ts, _ROW_ID)
            )
    ...
    def save_bluegreen_state(self, state) -> bool:
        ...
        try:
            ...
            logger.info("[ADMIN_STATE] État blue/green sauvegardé")
            return True
        except Exception as e:
    ...
    def mark_sync_completed(self) -> bool:
        ...
        try:
            self._run(
                "UPDATE {} SET last_sync_timestamp = NOW(), updated_at = NOW() WHERE id = %s".format(_TABLE),
                (_ROW_ID,)
            )
