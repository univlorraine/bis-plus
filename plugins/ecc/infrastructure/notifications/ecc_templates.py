...
class ECCNotificationTemplates(BaseTemplates):
    ...
    def render_success(cls, context: Dict[str, Any]) -> str:
        ...
        info_grid = cls.build_info_grid([
            ('DAG :', f'<strong>{dag_id}</strong>'),
            ('Date :', execution_date),
            ('Durée :', f'<strong>{duration}</strong>'),
            ('Tables :', f'<strong>{len(tables_imported)}</strong> table(s)'),
            ('Lignes source :', f'<strong>{total_fetched:,}</strong> ligne(s) récupérée(s)'),
            ('Lignes DB :', f'<strong>{total_rows:,}</strong> ligne(s) insérée(s)'),
            ('Mises à jour :', f'<strong>{total_updated:,}</strong> ligne(s) mise(s) à jour'),
            ('Ignorées :', f'<strong>{total_skipped:,}</strong> ligne(s) protégée(s) sifac_plus'),
            ('Statut :', (
    ...
    def render_error(cls, context: Dict[str, Any]) -> str:
        ...
        info_grid = cls.build_info_grid([
            ('DAG :', f'<strong>{dag_id}</strong>'),
            ('Tâche :', f'<strong>{task_id}</strong>'),
            ("Type d'erreur :", f'<code>{error_type}</code>'),
            ('Statut :', (
                '<span class="badge" style="background: #fff3e0; color: #ef6c00;">ECC</span>'
