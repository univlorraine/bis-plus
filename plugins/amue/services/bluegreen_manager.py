...
class BlueGreenManager:
    ...
    def mark_switch_completed(self) -> bool:
        ...
        state.active_schema = state.inactive_schema
        state.inactive_schema = old_active

        # Mise à jour des flags
        state.rollback_available = True
        state.rollback_schema = old_active
        state.last_switch_timestamp = datetime.now().isoformat()

        logger.info(f"[BLUEGREEN] Switch effectué: {old_active} -> {state.active_schema}")
    ...
    def mark_rollback_completed(self) -> bool:
        ...
        # Désactive le rollback
        state.rollback_available = False
        state.rollback_schema = ""

        logger.info(f"[BLUEGREEN] Rollback effectué: {old_active} -> {state.active_schema}")
        return self._save_state(state)
    ...
    def acquire_import_lock(self, correlation_id: str = "") -> bool:
        ...
        state.import_started_at = datetime.now().isoformat()
        state.import_correlation_id = correlation_id
