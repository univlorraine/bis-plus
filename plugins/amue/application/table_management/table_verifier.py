...
from amue.domain.fingerprint_comparator import (
    check_fingerprint_changes,
    compute_structure_diff,
    format_pg_type,
)
from amue.domain.structure_fetcher import split_column_defs
from amue.application.api_source_factory import get_structure_fetcher
...
class AMUETableVerifier:
    ...
    def __init__(self, api_hook, postgres_hook: SqlExecutor = None, target_schema: str = None):
        if api_hook is None:
        ...
        self._fetcher = get_structure_fetcher(api_hook)
    ...
    def verify_structure(self, table_info: Dict) -> Dict:
        ...
        logger.debug(f"[STRUCTURE_CHECK] table_info keys: {list(table_info.keys())}")
        logger.debug(f"[STRUCTURE_CHECK] table_info['primary_key'] = '{table_info.get('primary_key', 'NOT_SET')}'")

        try:
            columns = self._fetch_structure(table_name)
            api_pks = self._fetch_primary_keys(table_name)
            logger.debug(f"[STRUCTURE_CHECK] PKs API: '{api_pks}'")

            config_pks = table_info.get('primary_key', '')
            logger.debug(f"[STRUCTURE_CHECK] PKs config: '{config_pks}'")
    ...
    def verify_table(self, table_info: Dict) -> Dict:
        ...
        if old_fp_local and new_fp_local and old_fp_local != new_fp_local:
            diff_detail = self._compute_structure_diff(table_name, structure_result.get('columns', []))
            changes.append(
