...
from amue.domain.exceptions import TableNotFoundError
from amue.infrastructure.notifications import NotificationService, send_failure_notification
...
class AMUETableFilter:
    ...
    def _enrich_table_config(self, table_config: Dict, current_status: Dict) -> Dict:
        ...
        logger.debug(f"[ENRICH] Table {enriched.get('table_name')}: Config originale primary_key='{table_config.get('primary_key', 'NOT_SET')}'")
        ...
        logger.debug(f"[ENRICH] Table {enriched['table_name']}: primary_key après setdefault='{pk_value}'")

        if not pk_value or not pk_value.strip():
            logger.debug(f"[ENRICH] Table {enriched['table_name']}: PKs absentes -> needs_pk_update=True")
            enriched['needs_pk_update'] = True
        else:
            logger.debug(f"[ENRICH] Table {enriched['table_name']}: PKs présentes='{pk_value}' -> needs_pk_update=False")
