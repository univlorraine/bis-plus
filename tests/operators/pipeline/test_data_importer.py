...
class TestParallelInsertion:
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_single_worker_uses_sequential(self, mock_postgres, mock_varmgr):
        """parallel_workers=1 utilise le chemin séquentiel."""
        importer = self._make_importer(mock_varmgr, mock_postgres, parallel_workers=1)
        assert importer.parallel_workers == 1

    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_multi_worker_config(self, mock_postgres, mock_varmgr):
        """parallel_workers>1 est bien stocké."""
        importer = self._make_importer(mock_varmgr, mock_postgres, parallel_workers=3)
...
class TestQueueBasedInsertion:
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_single_worker_pipeline(self, mock_postgres, mock_varmgr):
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_multi_worker_pipeline(self, mock_postgres, mock_varmgr, mock_create_hook):
        """workers=2, 12 rows, batch_size=5 -> tous les batches traites."""
        importer = self._make_importer(mock_varmgr, mock_postgres, parallel_workers=2, batch_size=5)
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_consumer_error_propagates(self, mock_postgres, mock_varmgr):
        """execute_batch raise au 2e batch -> AMUEBatchError propagee via f.result()."""
        from amue.exceptions import AMUEBatchError, AMUEDatabaseError
        importer = self._make_importer(mock_varmgr, mock_postgres, parallel_workers=1, batch_size=5)
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_producer_error_propagates(self, mock_postgres, mock_varmgr):
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_empty_stream(self, mock_postgres, mock_varmgr):
        """0 rows -> retourne (0, 0, [])."""
        importer = self._make_importer(mock_varmgr, mock_postgres, parallel_workers=1, batch_size=5)
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_meta_columns_appended(self, mock_postgres, mock_varmgr):
    ...
    @patch('amue.operators.pipeline.data_importer.VarMgr')
    @patch('amue.operators.pipeline.data_importer.PostgresHook')
    def test_multi_worker_connections_closed(self, mock_postgres, mock_varmgr, mock_create_hook):
        """workers=3, verifie close_connection() appele 3 fois."""
        importer = self._make_importer(mock_varmgr, mock_postgres, parallel_workers=3, batch_size=5)
