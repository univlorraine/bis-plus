...
from common.infrastructure.config.airflow_helpers import AirflowVariableManager
...
class TestAirflowVariableManagerGetInt:
    """Tests pour AirflowVariableManager.get_int."""

    def test_returns_default_when_missing(self):
        with patch.dict('sys.modules', {'airflow.sdk': None, 'airflow.models': None}):
            result = AirflowVariableManager.get_int("missing", default=42)

        assert result == 42

    def test_parses_string_value(self):
        mock_sdk_module = MagicMock()
        mock_sdk_module.Variable.get.return_value = "10"

        with patch.dict('sys.modules', {'airflow.sdk': mock_sdk_module}):
            result = AirflowVariableManager.get_int("key", default=1)
    ...
    def test_raises_on_non_integer_value(self):
        ...
        mock_sdk_module = MagicMock()
        mock_sdk_module.Variable.get.return_value = "not_a_number"

        with patch.dict('sys.modules', {'airflow.sdk': mock_sdk_module}):
            with pytest.raises(AirflowException, match="n'est pas un entier"):
                AirflowVariableManager.get_int("key", default=1)

    def test_raises_when_below_min_value(self):
        from airflow.exceptions import AirflowException

        mock_sdk_module = MagicMock()
        mock_sdk_module.Variable.get.return_value = "0"

        with patch.dict('sys.modules', {'airflow.sdk': mock_sdk_module}):
            with pytest.raises(AirflowException, match=">= 1"):
                AirflowVariableManager.get_int("key", default=1, min_value=1)

    def test_raises_when_above_max_value(self):
        from airflow.exceptions import AirflowException

        mock_sdk_module = MagicMock()
        mock_sdk_module.Variable.get.return_value = "100000"

        with patch.dict('sys.modules', {'airflow.sdk': mock_sdk_module}):
            with pytest.raises(AirflowException, match="<= 65535"):
                AirflowVariableManager.get_int("key", default=1, max_value=65535)

    def test_accepts_value_within_bounds(self):
        mock_sdk_module = MagicMock()
        mock_sdk_module.Variable.get.return_value = "25"

        with patch.dict('sys.modules', {'airflow.sdk': mock_sdk_module}):
            result = AirflowVariableManager.get_int("key", default=1, min_value=1, max_value=65535)
